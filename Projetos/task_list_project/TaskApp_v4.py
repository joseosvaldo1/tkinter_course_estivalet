...
class TaskApp:
    ...
    def create_widgets(self):
        ...
        self.title_label = tk.Label(
            self.root, text="Lista de Tarefas", font=("Arial", 16, "bold"),
            bg="#f4f4f4"
        )
        self.title_label.pack(pady=10)

        # Frame para entrada da tarefa
        self.input_frame = tk.Frame(self.root, bg="#f4f4f4")
        self.input_frame.pack(pady=5, fill=tk.X, padx=10)

        self.task_entry = tk.Entry(
            self.input_frame, width=30, font=("Arial", 12), bg=entry_bg
        )
        self.task_entry.pack(side=tk.LEFT, padx=5, ipady=5, expand=True, fill=tk.X)

        self.add_button = tk.Button(
            self.input_frame, text="Adicionar Tarefa", command=self.add_task,
    ...
    def add_task(self):
        ...
        task_checkbox = tk.Checkbutton(
            task_frame, variable=task_var, command=lambda: self.toggle_task(task_checkbox, task_label, task_var),
            bg="#ffffff"
        )
        ...
        edit_button = tk.Button(
            task_frame, image=self.edit_icon, command=lambda: self.open_edit_window(task_label), bd=0, bg="#ffffff"
        )
        edit_button.pack(side=tk.LEFT, padx=5)

        delete_button = tk.Button(
            task_frame, image=self.delete_icon, command=lambda: self.delete_task(task_frame), bd=0, bg="#ffffff"
        )
        delete_button.pack(side=tk.LEFT, padx=5)

        task_frame.pack(fill=tk.X, pady=2)
        self.tasks.append({"frame": task_frame, "text": task_text, "label": task_label, "var": task_var})

        self.task_entry.delete(0, tk.END)

    def toggle_task(self, checkbox, label, var):
        """Marca a tarefa como concluída ou não, alterando o estilo do texto."""
        if var.get():
    ...
    def open_edit_window(self, label):
        ...
        save_button = tk.Button(
            button_frame, text="Salvar", bg="#34A853", fg="white", font=("Arial", 10, "bold"),
            command=lambda: self.save_task_edit(label, task_edit_entry.get(), edit_window)
        )
        save_button.pack(side=tk.LEFT, padx=5)

        cancel_button = tk.Button(
            button_frame, text="Cancelar", bg="#EA4335", fg="white", font=("Arial", 10, "bold"),
            command=edit_window.destroy
        )
        cancel_button.pack(side=tk.LEFT, padx=5)

    def save_task_edit(self, label, new_text, edit_window):
        """Salva a edição da tarefa, atualiza a lista tasks e fecha a janela."""
        if new_text.strip():
    ...
    def delete_task(self, frame):
        """Remove a tarefa da interface e da lista tasks."""
        for task in self.tasks:
            if task["frame"] == frame:
